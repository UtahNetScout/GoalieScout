...
def sync(db_path, source, season):
    """Run the automated data synchronization pipeline."""
    from ..scraping.pipeline.daily_update import DailyUpdatePipeline

    try:
        db = GoalieDatabase(db_path)
        pipeline = DailyUpdatePipeline(db=db, season=season)
...
@click.option('--min-games', default=5, show_default=True, help='Minimum games played')
@click.option('--season', default=None, help='Season slug (e.g. 2025-2026)')
@click.option('--db-path', default='./data/goalie_database.json', help='Path to database file')
def discover(league, min_games, season, db_path):
    """Find and store goalie prospects from configured leagues."""
    ...
    try:
        ...
        discovered = GoalieDiscovery(
            db=db,
            min_games=min_games,
            leagues=leagues,
        ).run(season)

        table = Table(title=f"Newly Discovered Goalies ({len(discovered)})")
        table.add_column("Name", style="cyan")
        table.add_column("League", style="green")
        table.add_column("Team", style="yellow")
        for goalie in discovered:
            table.add_row(
                str(goalie.get("name", "")),
                str(goalie.get("league", "")),
                str(goalie.get("team", "")),
            )
        console.print(table)
    except Exception as e:
...
def pipeline_status():
    """Show pipeline health and recent run information."""
    from ..scraping.pipeline.health_check import PipelineHealthCheck

    try:
        health = PipelineHealthCheck()
        console.print(Panel(health.get_status_summary(), title="Pipeline Health"))
    except Exception as e:
...
def validate_data(db_path):
    """Validate every goalie record in the JSON database."""
    from ..scraping.validation.validators import GoalieDataValidator

    try:
        goalies = GoalieDatabase(db_path).get_all_goalies()
        ...
        for goalie in goalies:
            ...
            flattened = {**record, **record.get("demographics", {})}
            metrics = record.get("performance_metrics", [])
            if metrics:
                flattened.update(metrics[0])
            result = validator.validate_record(flattened)
            status = "PASS" if result.passed else "FAIL"
            issues = "; ".join((result.errors + result.warnings)[:2])
            table.add_row(goalie.demographics.name, status, issues[:80])
