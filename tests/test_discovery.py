...
class TestGoalieDiscovery(unittest.TestCase):
    ...
    def test_new_player_is_persisted_to_real_database(self):
        """Discovery creates a profile through the real data package."""
        with tempfile.TemporaryDirectory() as temp_dir:
            db = GoalieDatabase(os.path.join(temp_dir, "goalies.json"))
            discovery = self._make_discovery(db=db)
            discovery._scraper = MagicMock()
            discovery._scraper.get_league_goalies.return_value = [{
            ...
            discovery.run("2023-2024")

            profile = db.get_goalie("persisted_player")
            self.assertIsNotNone(profile)
            self.assertEqual(profile.current_team, "Test Team")
